    ...
    <Popover open={isOpen} onOpenChange={handleOpenChange}>
      ...
      <PopoverContent className="w-full max-w-sm sm:max-w-sm p-0 shadow-2xl rounded-lg mt-3 z-50 relative bg-white border border-gray-200">
        ...
        <div className="p-2 sm:p-3 max-h-[70vh] overflow-y-auto">
          {/* Streak information */}
          <div className="flex items-center justify-between mb-2 sm:mb-3 bg-gradient-to-r from-[#32CD32] via-[#7CFC00] to-[#90EE90] rounded-lg p-2 sm:p-3">
            <div className="flex items-center">
              <Award className="text-white w-5 h-5 sm:w-6 sm:h-6" />
              <div className="ml-2 sm:ml-3">
                <div className="text-xs sm:text-sm font-medium text-white">Current Streak</div>
                <div className="text-lg sm:text-xl font-bold text-white">{checkInData?.currentStreak || 0} days</div>
              </div>
            </div>
            <div className="text-right">
              <div className="text-xs sm:text-sm font-medium text-white">Max Streak</div>
              <div className="text-lg sm:text-xl font-bold text-white">{checkInData?.maxStreak || 0} days</div>
            </div>
          </div>

          {/* Month selector */}
          <div className="flex items-center justify-between mb-2 sm:mb-3">
            <div className="flex items-center bg-gradient-to-r from-green-200 to-green-300 rounded-full px-2 sm:px-3 py-1">
              <ChevronLeftIcon size={14} className="sm:w-4 sm:h-4 text-green-800 cursor-pointer mr-1" />
              <span className="font-medium text-xs sm:text-sm text-green-900">{currentMonth}</span>
              <ChevronRightIcon size={14} className="sm:w-4 sm:h-4 text-green-800 cursor-pointer ml-1" />
            </div>
                ...
                <div key={weekIndex} className="grid grid-cols-7 gap-1">
                  {week.map((day, dayIndex) => {
                    ...
                    if (day.month === 'prev' || day.month === 'next') {
                      ...
                      dayClasses += "bg-gradient-to-r from-[#32CD32] to-[#7CFC00] text-white";
                    } else if (day.isCheckedIn) {
                      dayClasses += "text-white bg-green-300";
                    } else {
                      dayClasses += "hover:bg-green-200";
                    }
                    ...
                    return (
                      <div key={`${weekIndex}-${dayIndex}`} className="flex justify-center">
                        <div className={dayClasses}>
                          {day.day}
                          {day.isCheckedIn && (
                            <CheckCircle size={8} className="sm:w-[10px] sm:h-[10px] absolute -top-1 -right-1 text-green-500" />
                          )}
                        </div>
                      </div>
                    );
          ...
          <div className="mt-2 sm:mt-3">
            <button
              ...
              className={`w-full py-2 px-4 rounded-full font-medium text-white flex items-center justify-center text-sm sm:text-base ${checkInData?.canCheckIn
                ? 'bg-gradient-to-r from-[#32CD32] via-[#7CFC00] to-[#90EE90] text-black hover:!bg-black hover:!from-black hover:!via-black hover:!to-black hover:text-white'
          ...
          <div className="mt-2 sm:mt-3 bg-gray-50 rounded-lg p-2 sm:p-3 text-xs sm:text-sm">
            ...
            <ul className="space-y-1 sm:space-y-2">
              <li className="flex items-center">
                <div className="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-green-500 flex items-center justify-center mr-2 text-xs font-bold text-white">1</div>
                <span>
              ...
              <li className="flex items-center">
                <div className="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-green-500 flex items-center justify-center mr-2 text-xs font-bold text-white">7</div>
                <span>
              ...
              <li className="flex items-center">
                <div className="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-green-500 flex items-center justify-center mr-2 text-xs font-bold text-white">30</div>
                <span>
